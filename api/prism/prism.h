...
typedef enum {
  ...
  PR_ERR_UNKNOWN,
  PR_ERR_TIMEOUT,
  PR_ERR_DEVICE_NOT_FOUND,
  PR_ERR_DEVICE_BUSY,
  PR_ERR_DEVICE_NOT_INITIALIZED,
  PR_ERR_DEVICE_NOT_READY,
  PR_ERR_DEVICE_COMMUNICATION,
  PR_ERR_DEVICE_NOT_RESPONDING,
  PR_ERR_DEVICE_NOT_SUPPORTED,
  PR_ERR_INVALID_DEVICE,
} prism_err;
...
#define PRISM_PIN_1LOW_DEFAULT 2 
#define PRISM_PIN_2LOW_DEFAULT 3 
#define PRISM_PIN_3LOW_DEFAULT 4 
#define PRISM_PIN_4LOW_DEFAULT 5 

// Timing (Taktleitung)
#define PRISM_PIN_CLK_DEFAULT 6 

// Entry-Flag (neuer Eintrag beginnt)
#define PRISM_PIN_NXT_DEFAULT 7 

// High-Byte: Datenleitungen Bit 4–7
#define PRISM_PIN_7HIGH_DEFAULT 8  
#define PRISM_PIN_8HIGH_DEFAULT 9  
#define PRISM_PIN_9HIGH_DEFAULT 10 
#define PRISM_PIN_10HIGH_DEFAULT 11

typedef struct prism_dev_config prism_dev_config_t;

typedef struct prism_dev_type  prdev_t;


extern prism_err prism_device_create(const uint8_t address, bool wireInit,
                              const prism_dev_config_t *config,
                              prdev_t *device);

extern prism_err prism_device_stop(const prdev_t *device);
extern prism_err prism_device_reset(const prdev_t *device);
